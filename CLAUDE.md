...
| `@claude close` | Used **only to summarize the issue for skill creation**: record the decisions made in this work under `docs/decisions/`, then propose the skill(s) worth creating for `mekhal/claudeskill`, including the concrete add-skill steps (see "Skills"). **Do NOT close the issue yourself — the human closes the issue manually.** | No |
...
3. **Store** — skills live in the **private repo [`mekhal/claudeskill`](https://github.com/mekhal/claudeskill)** — the source of truth. They are **not** stored in this public demo repo.
4. **Reuse** — the agent invokes these skills in later loops to work better and stay consistent with prior human decisions.

### Store vs runtime

- **Store:** the private `mekhal/claudeskill` repo holds the skill files.
- **Runtime:** Claude Code auto-discovers skills from `.claude/skills/` on the runner's filesystem. It **cannot reach into the private repo on its own** — the skills must first be brought onto the runner. Wiring that up (checkout step / submodule / token in the workflow) is tracked separately and is **out of scope here**.
...
When you handle `@claude close`, propose the skill(s) worth creating and include, for each: a draft `SKILL.md` (in the format above) and the steps to add it to `mekhal/claudeskill` — create the `.claude/skills/<kebab-name>/SKILL.md` file with the frontmatter and body, then commit it to the `claudeskill` repo.

### Using a skill

Skills are stored in `mekhal/claudeskill` (source of truth). Before starting any piece of work, check the skills available on the runner and invoke the relevant skill first.
